...
- **Global scope**: `/workspaces/94lama/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/workspaces/94lama/.opencode/notes/`)
...
2. Glob `/workspaces/94lama/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (/workspaces/94lama/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `/workspaces/94lama/.opencode/notes/` — works in any directory
