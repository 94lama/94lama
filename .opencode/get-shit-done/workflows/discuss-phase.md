...
<required_reading>
@/workspaces/94lama/.opencode/get-shit-done/references/domain-probes.md
@/workspaces/94lama/.opencode/get-shit-done/references/gate-prompts.md
@/workspaces/94lama/.opencode/get-shit-done/references/universal-anti-patterns.md
</required_reading>
...
```bash
INIT=$(node "/workspaces/94lama/.opencode/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_ADVISOR=$(node "/workspaces/94lama/.opencode/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-advisor 2>/dev/null)
```
...
- Read and execute @/workspaces/94lama/.opencode/get-shit-done/workflows/discuss-phase-power.md end-to-end
...
```bash
TODO_MATCHES=$(node "/workspaces/94lama/.opencode/get-shit-done/bin/gsd-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "/workspaces/94lama/.opencode/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @/workspaces/94lama/.opencode/agents/gsd-advisor-researcher.md for your role and instructions.
...
```bash
MAX_PASSES=$(node "/workspaces/94lama/.opencode/get-shit-done/bin/gsd-tools.cjs" config-get workflow.max_discuss_passes 2>/dev/null || echo "3")
```
...
```bash
node "/workspaces/94lama/.opencode/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "/workspaces/94lama/.opencode/get-shit-done/bin/gsd-tools.cjs" state record-session \
...
```bash
node "/workspaces/94lama/.opencode/get-shit-done/bin/gsd-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
**Full step-by-step instructions:** @/workspaces/94lama/.opencode/get-shit-done/workflows/discuss-phase-power.md
