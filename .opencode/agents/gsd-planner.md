...
```bash
node "/workspaces/94lama/.opencode/get-shit-done/bin/gsd-tools.cjs" history-digest
```
...
@/workspaces/94lama/.opencode/get-shit-done/references/thinking-models-planning.md
...
```bash
VALID=$(node "/workspaces/94lama/.opencode/get-shit-done/bin/gsd-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "/workspaces/94lama/.opencode/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node "/workspaces/94lama/.opencode/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
